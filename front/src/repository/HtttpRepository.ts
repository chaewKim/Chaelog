...
export default class HttpRepository {
  ...
  public post<T>(config: HttpRequestConfig, clazz: { new (...args: any[]): T } | null = null): Promise<T> {
    return this.httpClient
      .request({ ...config, method: 'POST' })
      .then((response) => plainToInstance(clazz !== null ? clazz : Null, response)) //null이 아니면 clazz쓰고 null이면 Null
  }
  public patch<T>(config: HttpRequestConfig, clazz: { new (...args: any[]): T } | null = null): Promise<T> {
    return this.httpClient
